...
{
if(i>=0&&i<10&&j>=0&&j<10&&mine_positions[i][j]!='X')
mine_positions[i][j]++;
}
 ...
 while(counter<10)
    {
        int i=rand()%10;
        int j=rand()%10;
        if(mine_positions[i][j]=='0'){
            mine_positions[i][j]='X';
...
{
if (table[i][j]=='*'&&mine_positions[i][j]!='X'&&i>=0&&i<10&&j>=0&&j<10)
    {
    table[i][j]=mine_positions[i][j];
...
cout
<<"Rules:"
<<endl<<"Enter 'o' , then enter value of i and j to open Cell of i th row and j th column."
